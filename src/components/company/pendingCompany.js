...
function ViewJobs(props) {
    ...
    async function getInactiveCompany() {
        let inactiveCompanyData = await CompanyService.fetchInactiveJobCompanies();
        setData(inactiveCompanyData.data.data.fetchedCompanies);
    }
        ...
        },
        {
            ...
            Cell: (row) => (
                <div className="btn-group">
                    ...
                    <button className="btn btn btn-success" type="button" onClick={() => goLive(row.original._id)} >
    ...
    const goLive = async (id) => {
        let bodyData = { status: true, companyId: id }
        let updateCompany = await CompanyService.updateJobCompany(bodyData);
        if (updateCompany.data.status) {
            props.history.push({
                pathname: JOBS_ADMIN + "/company/activeCompany/",
            })
        }
    }
