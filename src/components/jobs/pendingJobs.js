...
function ViewJobs(props) {
    ...
    async function getInactiveJobs() {
        let inActiveJobsData = await JobsService.fetchInactiveJobs();
        setData(inActiveJobsData.data.data.fetchedJobs);
    }
    ...
    const goLive = async (id) => {
        let bodyData = { isActive: true, jobId: id }
        let updateJob = await JobsService.updateJob(bodyData);
        if (updateJob.data.status) {
            props.history.push({
                pathname: JOBS_ADMIN + "/jobs/activeJobs/",
            })
        }
    }
